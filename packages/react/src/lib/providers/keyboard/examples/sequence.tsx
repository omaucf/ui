...
import { useHotkeys } from "@veehance/react/keyboard";
  ...
  return (
    <div className="flex w-full max-w-[26rem] flex-col gap-4 rounded-[0.875rem] border border-default bg-elevated p-5 text-accented text-sm shadow-sm">
      ...
      <div className="flex items-baseline gap-2 border-default border-t pt-3.5">
        <span className="text-muted">Current page</span>
        <span
          className="inline-flex items-center gap-1.5 rounded-full bg-accented px-2 py-[0.1875rem] font-semibold text-accent text-xs data-[state=active]:bg-accent-subtle"
          data-state="active"
        >
          {page}
        </span>
