...
describe("enableOnFormTags", () => {
  ...
  it("should ignore a single-key hotkey while typing in an input", async () => {
    ...
    render(() => (
      <Form onModifier={vi.fn()} onOptIn={vi.fn()} onPlain={onPlain} />
    ));
  ...
  it("should fire a single-key hotkey in an input when opted in", async () => {
    ...
    render(() => (
      <Form onModifier={vi.fn()} onOptIn={onOptIn} onPlain={vi.fn()} />
    ));
...
describe("conflictBehavior", () => {
  ...
  it("should drop the earlier registration when replacing", async () => {
    ...
    expect(screen.getByTestId("ids").textContent).not.toContain("first");
  });
    ...
    return (
      <div>
        ...
        <span data-testid="state">
          {recorder.state().recording ? "recording" : "idle"}
        </span>
        <span data-testid="value">
          {recorder.state().value?.display ?? "empty"}
        </span>
      </div>
    );
  };
