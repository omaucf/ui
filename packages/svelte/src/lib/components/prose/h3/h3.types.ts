import type theme from "#build/theme/prose/h3";
import type { ComponentApi } from "#build/ui/types";

import type {
  HTMLProps,
  PolymorphicProps,
  RefAttribute,
} from "$lib/types/element.js";

export interface ProseH3BaseProps extends PolymorphicProps<"h3">, RefAttribute {
  anchor?: boolean;
}

export interface ProseH3Props extends ProseH3BaseProps, HTMLProps<"h3"> {
  ui?: Partial<ProseH3Theme["slots"]>;
}

export type ProseH3Theme = ComponentApi<typeof theme, "h3", "ui:prose">;
