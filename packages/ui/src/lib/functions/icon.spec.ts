...
describe("#generateIcons", () => {
  ...
  it("generates imports for vscode-icons when prose argument is provided", () => {
    const result = generateIcons({ prose: true });

    expect(result).toContain("@iconify-json/vscode-icons/icons.json");
    expect(result).toContain("vscodeIcons");
  });

  it("generates multiple icon imports when provided", () => {
    const result = generateIcons({
      ui: {
        icons: { dark: "lucide:moon", github: "mdi:github" },
      },
    });
